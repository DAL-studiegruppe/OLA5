...
# normalt virker read.csv ikke godt med store csv-filer, derfor anvendes her package data.table.
# funktion "fread" er den samme som read.csv men hurtigere, al kontrol i read.csv som nrows, colclasses og sep blive håndtere automatisk
# encoding "Latin-1" håndterer danske bogstaver æ, ø, å
...
# for at lave en grafisk illustration af fordelingen af svarene på spørgsmålet skal vi have en frekvens-tabel
...
ggplot(svar_df, aes(x = Svar, y = Antal)) +
  ...
  geom_text(aes(label = round(Antal, 2)), position = position_dodge(width = 0.9), vjust = -0.5) + 
  theme_minimal() +
  labs(title = "Flertallet synes der er gode muligheder for at låne penge til sin virksomhed",
       x = "Svarmulighed",
       y = "Antal svar") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), plot.title = element_text(size = 10))

# for uddybe opg.1 mht. Baums artikler
# vi vil gerne se hvad har folk svaret ift. størrelse af virksomhed og det gør vi ved at vi deler virksomhedens størrelse med antal af ansatte og svarmuligheder med nettotal
df1.1 <- df %>%
  mutate(`Hvordan ser du mulighederne for at låne penge til din virksomhed? (fiktivt spørgsmål)` = case_when(
    `Hvordan ser du mulighederne for at låne penge til din virksomhed? (fiktivt spørgsmål)` == "Meget gode" ~ 1,
    `Hvordan ser du mulighederne for at låne penge til din virksomhed? (fiktivt spørgsmål)` == "Gode" ~ 0.5,
...
# omdanne til factor med levels for en specifik rækkefølge
...
# Nu vil vi gerne se med et økonomisk perpektiv siden antallet af medarbejdere ikke alene er afgørende lånemulighederne
# Vi ved virksomheder med høj indtjening på aktiver nemmere kan låne penge. Mere robuste virksomheder har bedre udsigter til lån
# Vi kigger derfor på afkastningsgrad og soliditetsgrad i de næste grafer
# Vi skal først have gennemsnit af afkastningsgrad og soliditetsgrad fordi kolonnerne er blevet delt fra 2016-2021
...
ggplot(df_long, aes(x = type, y = værdi, fill = svar_kategorii)) +
  ...
  geom_text(aes(label = round(værdi, 2)), position = position_dodge(width = 0.9),
            vjust = -0.5) +  
  labs(title = "Virksomheder med en højere soliditetsgrad synes\nbedre om finansieringsklimaet",
       y = "Pct.",
       x = "",
       fill = "") +
  theme_minimal() +
...
ggplot(df_3g, aes(x = svar_kategori, y = Balance, fill = svar_kategori)) +
  ...
  labs(title = "Virksomheder med en højere balance\nsynes bedre om finansieringsklimaet",
       y = "log(balance)",
       x = "",
       fill = "") +
  theme_minimal() +
